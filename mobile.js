...
(function MobileUI() {
    ...
    /* ══════════════════════════════════════════════════════════
       SIDEBAR CLÁSICO (PC) — hamburguesa en móvil
       Gestiona el sidebar original del ERP (.sidebar + #sidebarOverlay)
       cuando la pantalla es ≤ 768px. El nuevo drawer (#mobileDrawer)
       es la UI principal móvil; este sidebar queda como fallback/legado.
       ══════════════════════════════════════════════════════════ */
    ...
    // Exponer para onclicks inline del HTML
    window.openMobileSidebar  = _openMobileSidebar;
    window.closeMobileSidebar = _closeMobileSidebar;
    window.toggleMobileSidebar = function() {
        const s = document.getElementById('sidebar');
        if (s && s.classList.contains('mobile-open')) _closeMobileSidebar();
        else _openMobileSidebar();
    };

    // Cerrar sidebar clásico al hacer resize a desktop
    ...
    // Swipe en el documento para abrir/cerrar el sidebar clásico
    (function _initClassicSidebarSwipe() {
        let _tx = 0, _ty = 0;
        ...
        document.addEventListener('touchend', function(e) {
            ...
            const s  = document.getElementById('sidebar');
            if (!s) return;
            // Swipe derecha → izquierda: cerrar
            if (dx < -60 && Math.abs(dy) < 60 && s.classList.contains('mobile-open')) {
                _closeMobileSidebar();
            }
            // Swipe izquierda → derecha desde el borde: abrir
            if (dx > 60 && Math.abs(dy) < 60 && _tx < 30 && !s.classList.contains('mobile-open')) {
                _openMobileSidebar();
            }
        }, { passive: true });
    })();
    ...
    /* ══════════════════════════════════════════════════════════
       mobileNavTo — navega y cierra el sidebar clásico
       Usado por los botones del bottom nav antiguo: mobileNavTo('pos')
       ══════════════════════════════════════════════════════════ */
    window.mobileNavTo = function(mod) {
        _closeMobileSidebar();
        if (typeof navigateTo === 'function') navigateTo(mod);
    };

    /* ══════════════════════════════════════════════════════════
       DEEP LINK — ?module=pos abre directamente un módulo
       Útil para PWA shortcuts y enlaces externos
       ══════════════════════════════════════════════════════════ */
    (function _handleDeepLink() {
        const _urlModule = new URLSearchParams(window.location.search).get('module');
        if (!_urlModule) return;
        const _tryNav = function(attempts) {
            const panel = document.getElementById('module-' + _urlModule);
            const appEl = document.getElementById('appMain');
            if (panel && appEl && appEl.classList.contains('visible')) {
                if (typeof navigateTo === 'function') navigateTo(_urlModule);
                history.replaceState({}, '', '/');
            } else if (attempts < 40) {
                setTimeout(function() { _tryNav(attempts + 1); }, 250);
            }
        };
        _tryNav(0);
    })();
